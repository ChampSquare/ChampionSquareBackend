...
from .models import Profile, Answer, AnswerPaper, User, TestCase, \
                         has_profile
...
from Unicorn.models import Student
from JeeMain.models import Result
...
def homepage(request):
    """The homepage for unaunthenticatd users """
    ci = RequestContext(request)
    return my_render_to_response(request, "home/index.html", {}, context_instance=ci)
...
def user_login(request):
    ...
    if user.is_authenticated():
        if user.groups.filter(name='moderator').count() > 0:
            return my_redirect('/exam/manage/')
        return my_redirect("/exam/start/1/")
