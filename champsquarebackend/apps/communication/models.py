...
class CommunicationEventType(TimestampedModel):
    """
        A 'type' of communication. Like an test participation event
    """
    code = AutoSlugField(
    ...
    QUIZ_RELATED = 'Quiz related'
    USER_RELATED = 'User related'
    CATEGORY_CHOICES = (
        (QUIZ_RELATED, _('Quiz related')),
        (USER_RELATED, _('User related'))
    )

    category = models.CharField(
        _('Category'), max_length=255, default=QUIZ_RELATED,
        choices=CATEGORY_CHOICES)

    # Template content for emails
    # NOTE: There's an intentional distinction between None and ''. None
    # instructs to look for a file-based template, '' is just an empty
    # template.
