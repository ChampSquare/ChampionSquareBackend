...
class Dispatcher(object):
    ...
    def __init__(self, logger=None, mail_connection=None):
        ...
        self.logger = logger
        # supply a mail_connection if you want the dispatcher to use 
    ...
    def create_email(self, user, messages, email):
        """
        Create ``Email`` instance in database for logging puposes
        """
    ...
    def send_user_email_messages(self, user, messages, attachments=None):
        """
        Send message to the registered user and collect data is database
        """
        if not user.email:
        ...
        if settings.SETTINGS_SAVE_SENT_EMAILS_TO_DB:
    ...
    def send_email_messages(self, recipient_email, messages,
        ...
        """
        from_email = from_email or settings.SETTINGS_FROM_EMAIL
