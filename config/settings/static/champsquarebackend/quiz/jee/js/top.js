	...
	jQuery.ajax({
		...
		success: function(data) {
			...
			if(validity.toLowerCase()!="live"){
				window.location.href ="error.html?E105"+((document.cookie.indexOf('isApp=true')>= 0)?"isApp":"");
			}
			if(url.indexOf("index.html") >= 0)
...
jQuery(document).ready(function(){
	...
	if(url.indexOf("quiz.html")>=0 || url.indexOf("instructions.html")>=0 || url.indexOf("FeedBack.html")>=0 ||url.indexOf("close.html")>=0){
		...
		if(assessmentId==null|| assessmentId.length==0 || orgId == null || orgId.length==0){
			window.location.href ="error.html"+((document.cookie.indexOf('isApp=true')>= 0)?"isApp":"");
		}
	}
...
//	Need to be change
	if(mockVar.storeCandResponse==1){
		quesText = quesText.replace(/tkc{{ URL_ROOT }}/static/Uscholar/img/g, xmlFilePath+ mockVar.availableQpId + '/tkcimages');
	}
	else{
		quesText = quesText.replace(/tkc{{ URL_ROOT }}/static/Uscholar/img/g, xmlFilePath + 'tkcimages');
	}
	return quesText;
}
...
var hintDivInnerHtml = '<span class="hintIcon" style="display:none;padding:20px"><span class="hint"><b>Hint : </b></span><img src="{{ URL_ROOT }}/static/Uscholar/img/info.gif" style="-moz-user-select: text; width: 20px" onclick="openHintPopUp(this)"></span><div style="display: none;" class="popUpDiv hintDiv" id=""><div class="popupHeader">Hint<div class="btnclose" style="-moz-user-select: text;" onclick="closeHintPopup()"><img src="{{ URL_ROOT }}/static/Uscholar/img/btn_close.gif" style="-moz-user-select: text;"></div></div><div class="hintText" style="padding: 5px"></div><div class="tip" style="-moz-user-select: text;"></div></div>';


//var mobhintDiv = '<span class="hintIcon"><img src="{{ URL_ROOT }}/static/Uscholar/img/info.gif" style="-moz-user-select: text; width: 20px" onclick="openHintPopUp(this)"></span><div style="display: none;" class="popUpDiv hintDiv" id=""><div class="popupHeader">Hint<div class="btnclose" style="-moz-user-select: text;" onclick="closeHintPopup()"><img src="{{ URL_ROOT }}/static/Uscholar/img/btn_close.gif" style="-moz-user-select: text;"></div></div><div id="hintText" style="padding: 5px"></div><div class="tip" style="-moz-user-select: text;"><img src="{{ URL_ROOT }}/static/Uscholar/img/tip1.gif" style="-moz-user-select: text;"></div></div>';
	...
	$.ajax({
		...
		success: function (data) {
			if(data!=null && data.Error != null){
				window.location.href="error.html?E103"+((document.cookie.indexOf('isApp=true')>= 0)?"isApp":"");
	...
	$.ajax({
		...
		dataType: ($.browser.msie) ? "text" : "xml",
				success: function(data) {
					...
					setTimeout(function(){
						window.location.href="error.html?E404"+((document.cookie.indexOf('isApp=true')>= 0)?"isApp":"");
					},50);
				}
	});
	return xml;
...
function quizPageHeight() {
    if(mockVar.storeCandResponse==1 && !($.trim(mockVar.mcQName).length>0)&&!($.trim(mockVar.msQName).length>0)&&!($.trim(mockVar.saQName).length>0)&&!($.trim(mockVar.fillBlankQName).length>0)&&
    ...
    !($.trim(mockVar.programingQName).length>0) && mockVar.showMarks==false && showHintDiv==false){
		$('.questionTypeCont').height(0);
	}
	if(mockVar.storeCandResponse==1 && mockVar.langCount<2 && mockVar.curQuesBean.quesType != "TYPING TEST"){
...
function basicDetails(xml,isExtraJSRequired,page){
	...
	if(mockVar.storeCandResponse==1){
		...
		if($(xml).find("USEDEFAULTCANDIDATEIMG").length>0 && $(xml).find("USEDEFAULTCANDIDATEIMG").text() == "NO"){
			...
			$(".candidateImg").attr("src","{{ URL_ROOT }}/static/Uscholar/img/NewCandidateImage.jpg");
		}
	}
	if(mockVar.storeCandResponse==0){
	...
	if(isExtraJSRequired && $(xml).find('AdditionalTools').length>0 && $.trim($(xml).find('AdditionalTools').text()).length>0){
		...
		for(i=0;i<$(xml).find('AdditionalTools').text().split(',').length;i++){
			...
			if(value=="textareaPad"){
				...
				if($(window).width()<768)
					$(".ruler-div img").attr('src','{{ URL_ROOT }}/static/Uscholar/img/measure_10.png');
				else
					$(".ruler-div img").attr('src','{{ URL_ROOT }}/static/Uscholar/img/measure_15.png');	
			}
			...
			$(this).find('subsection').each(function(){
				...
				$(this).find("question").each(function(){
					if(noOfTypingQues==0){
						...
						var keyboardType = '', typingType = '', answerType = '', isCaseSensitive = false, isEvaluationRequired = false, SARecordingTime='0';
						...
						if(quesType == "MCQ" || quesType == "MSQ"){
							...
							$(this).find('languageSpecificData').find('lang').each(function(){
								...
								$(this).find('allOptions').find('option').each(function(){
									...
									if(optText.indexOf("tkc{{ URL_ROOT }}/static/Uscholar/img/")!=-1){
										/*tempImageTag = optText.split("tkc{{ URL_ROOT }}/static/Uscholar/img/");
									tempImageName=tempImageTag[1].split(".")[0];
									tempImageName=tempImageName.replace(/\s/g, "_");
									optText=tempImageTag[0]+"tkc{{ URL_ROOT }}/static/Uscholar/img/"+tempImageName+"."+tempImageTag[1].split(".")[1];*/
										tempImageTag = optText.split("tkc{{ URL_ROOT }}/static/Uscholar/img/");
										for(var k=1;k<tempImageTag.length;k++){
											...
											if(temporaryFileExtension == "mp4"){
												optText = optText + "<div class='jwAudioVideo' id='jwVideo_"+quesId+"_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+tempImageName+"'></div>";
												oldFileCounter++ ;
											}else if(temporaryFileExtension == "mp3"){
												optText = optText + "<div class='jwAudioVideo' id='jwAudio_"+quesId+"_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+tempImageName+"'></div>";
												oldFileCounter++ ;
											}
										}
									}
									if(optText.indexOf("<audio>")!=-1){
										...
										for(var i=0;i<newAudioFile.length;i++){
											while (optText.indexOf(newAudioFile[i])!=-1) {
												optText = optText.replace(newAudioFile[i],"<div class='jwAudioVideo' id='jwAudio_"+quesId+"_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+newAudioFile[i].split("<audio>")[1]+"'></div>");
												oldFileCounter++ ;
											}
										}
									}
									if(optText.indexOf("<video>")!=-1){
										...
										for(var i=0;i<newVideoFile.length;i++){
											while (optText.indexOf(newVideoFile[i])!=-1) {
												optText = optText.replace(newVideoFile[i],"<div class='jwAudioVideo' id='jwVideo_"+quesId+"_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+newVideoFile[i].split("<video>")[1]+"'></div>");
												oldFileCounter++ ;
											}
										}
									}
	...
	if(parseInt(isResumed) == 0){
		...
		$(xml).find('comprehensions').find('comprehension').each(function(){
			...
			$(this).find('lang').each(function(){
				...
				if(quesText.indexOf("tkc{{ URL_ROOT }}/static/Uscholar/img/")!=-1){
					tempImageTag = quesText.split("tkc{{ URL_ROOT }}/static/Uscholar/img/");
					for(var k=1;k<tempImageTag.length;k++){
						//var temporaryImageName = "";
						//temporaryImageName = tempImageTag[k];
						tempImageName=tempImageTag[k].split(".")[0];
						...
						if(temporaryFileExtension == "mp4"){
							quesText = quesText + "<div class='jwAudioVideo' id='jwVideo_"+compreId+"_Claq_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+tempImageName+"'></div>";
							oldFileCounter++ ;
						}else if(temporaryFileExtension == "mp3"){
							quesText = quesText + "<div class='jwAudioVideo' id='jwAudio_"+compreId+"_Claq_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+tempImageName+"'></div>";
							oldFileCounter++ ;
						};
						//quesText=tempImageTag[0]+"tkc{{ URL_ROOT }}/static/Uscholar/img/"+tempImageName+temporaryImageName.substring(temporaryImageName.indexOf("."));
					};
				}
				if(quesText.indexOf("<audio>")!=-1){
					...
					for(var i=0;i<newAudioFile.length;i++){
						while (quesText.indexOf(newAudioFile[i])!=-1) {
							quesText = quesText.replace(newAudioFile[i],"<div class='jwAudioVideo' id='jwAudio_"+compreId+"_Claq_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+newAudioFile[i].split("<audio>")[1]+"'></div>");
							oldFileCounter++ ;
						}
					}
				}
				if(quesText.indexOf("<video>")!=-1){
					...
					for(var i=0;i<newVideoFile.length;i++){
						while (quesText.indexOf(newVideoFile[i])!=-1) {
							quesText = quesText.replace(newVideoFile[i],"<div class='jwAudioVideo' id='jwVideo_"+compreId+"_Claq_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+newVideoFile[i].split("<video>")[1]+"'></div>");
							oldFileCounter++ ;
						}
					}
				}
			...
			$(this).find('lang').each(function(){
				...
				if(quesText.indexOf("tkc{{ URL_ROOT }}/static/Uscholar/img/")!=-1){
					...
					for(var k=1;k<tempImageTag.length;k++){
						...
						quesText = quesText.replace(temporaryFileName,tempImageName);
						if(temporaryFileExtension == "mp4"){
							quesText = quesText + "<div class='jwAudioVideo' id='jwVideo_"+laqId+"_Claq_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+tempImageName+"'></div>";
							oldFileCounter++ ;
						}else if(temporaryFileExtension == "mp3"){
							quesText = quesText + "<div class='jwAudioVideo' id='jwAudio_"+laqId+"_Claq_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+tempImageName+"'></div>";
							oldFileCounter++ ;
						}
						//quesText=tempImageTag[0]+"tkc{{ URL_ROOT }}/static/Uscholar/img/"+tempImageName+temporaryImageName.substring(temporaryImageName.indexOf("."));
					}
				...
				if(quesText.indexOf("<audio>")!=-1){
					...
					for(var i=0;i<newAudioFile.length;i++){
						while (quesText.indexOf(newAudioFile[i])!=-1) {
							quesText = quesText.replace(newAudioFile[i],"<div class='jwAudioVideo' id='jwAudio_"+laqId+"_Claq_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+newAudioFile[i].split("<audio>")[1]+"'></div>");
							oldFileCounter++ ;
						}
					}
				}
				if(quesText.indexOf("<video>")!=-1){
					...
					for(var i=0;i<newVideoFile.length;i++){
						while (quesText.indexOf(newVideoFile[i])!=-1) {
							quesText = quesText.replace(newVideoFile[i],"<div class='jwAudioVideo' id='jwVideo_"+laqId+"_Claq_"+oldFileCounter+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/"+newVideoFile[i].split("<video>")[1]+"'></div>");
							oldFileCounter++ ;
						}
					}
				}
...
function fillSEQQues(compQues){
	...
	str +='<div align="right" style="float:right;"><a  href="javascript:;" id="scrollToBottom" title="Scroll Down" onclick="scrollToBottom()"><img width="28" height="28" align="right" alt="Scroll Down" src="{{ URL_ROOT }}/static/Uscholar/img/Down.png" style=""></a></div>';
	...
	str += '</ol></div></div></div><div align="right" style="float:right;clear:both"><a onclick="scrollToTop()" title="Scroll Up" id="scrollToTop"><img width="30" height="30" alt="Scroll Up" style="" src="{{ URL_ROOT }}/static/Uscholar/img/Up.png"></a></div>';
	str +='</div></div>';
	return str;
}
...
function fillMTCQues(compQues,columnType,childQues){
	...
	if(columnType === "drag"){
		...
		str += '</ul></div></div></div><div align="right" style="float:right;clear:both"><a onclick="scrollToTop()" title="Scroll Up" id="scrollToTop"><img width="30" height="30" alt="Scroll Up" style="" src="{{ URL_ROOT }}/static/Uscholar/img/Up.png"></a></div>';
		str +='</div></div>';
...
function fillFITBQues(compQues){
	...
	str += '<div align="right" style="float:right;clear:both"><a onclick="scrollToTop()" title="Scroll Up" id="scrollToTop"><img width="30" height="30" alt="Scroll Up" style="" src="{{ URL_ROOT }}/static/Uscholar/img/Up.png"></a></div>';
	str +='</div></div>';
	return str;
}
...
function fillBlankSA(compQuesStr,replaceStr,answer,curQuestion){
	...
	//str +='<img src="{{ URL_ROOT }}/static/Uscholar/img/keyboard.png" alt="Display virtual keyboard interface" class="keyboardInputInitiator" title="Select keyboard layout">';
...
function fillProgramingQues(ques){
	...
	str += '<div id="maximizeIcon" style="float: right;display:block;"><img src="{{ URL_ROOT }}/static/Uscholar/img/icon_maximum.png" id="maximg" style="cursor:pointer;" title="Maximize" onclick="showMaxVersion();"></div>';
	...
	str += '<div style="float:right;margin-top:-26px;margin-right:38px;"><img alt="" src="{{ URL_ROOT }}/static/Uscholar/img/Prog_copy.png" onclick="copyCode();" style="cursor:pointer;width:20px;" title="Copy the content" class="progImg"></div>';
	str +='<div style="float:right;margin-top:-26px;margin-right:12px;"><img alt="" src="{{ URL_ROOT }}/static/Uscholar/img/ChangeEditor.png" onclick="changeEditor();" style="cursor:pointer;width:20px;" title="Change the Editor" class="progImg"></div>';
	...
	str += '<div style="float:right;margin-top:-26px;margin-right:38px;"><img alt="" src="{{ URL_ROOT }}/static/Uscholar/img/Prog_copy.png" onclick="copyCode();" style="cursor:pointer;width:20px;" title="Copy the content" class="progImg"></div>';
	str +='<div style="float:right;margin-top:-26px;margin-right:12px;"><img alt="" src="{{ URL_ROOT }}/static/Uscholar/img/ChangeTextArea.png" onclick="changeTextArea();" style="cursor:pointer;width:20px;" title="Change the Textarea" class="progImg"></div>';
	...
	str +='<div id="maxMinTestCases1" style="margin: auto;display: none;"><img src="{{ URL_ROOT }}/static/Uscholar/img/icon_minimum.png" id="minimg" style="cursor:pointer;display:inline;" title="Minimize" onclick="maxToMin()"><table id="testCaseDisplayTable" width="99.5%" border="1" cellSpacing="0" style="border-color:#2F72B7;" class="testCaseTable"><thead><tr style="background:#2F72B7;color:white;text-align:center"><th style="width:10%;">TestCase No</th><th style="width:40%;">Inputs</th><th style="width:25%;">Expected output</th><th style="width:25%;">Result</th></tr></thead><tbody><tr><td>1</td><td>Inputs for TestCase 1</td><td>Output</td><td style="font-weight: bold;"><span style="color:green;font-weight: bold;">Pass</span> / <span style="color:red;font-weight: bold;">Fail</span></td></tr><tr><td>2</td><td>Inputs for TestCase 2</td><td>Output</td><td style="font-weight: bold;"><span style="color:green;font-weight: bold;">Pass</span> / <span style="color:red;font-weight: bold;">Fail</span></td></tr></tbody></table></div>';
	return str;
}
...
function quesContent(ques){
	...
	if(ques.quesType == "MCQ" && ques.comprehensionId == 0 && ques.laqId == 0){
		...
		str += "<div  class='leftDiv' style='width:99.6%'>";
		str +='<div align="right" style="float:right;"><a id="scrollToBottom" title="Scroll Down" onclick="scrollToBottom()"><img width="28" height="28" align="right" alt="Scroll Down" src="{{ URL_ROOT }}/static/Uscholar/img/Down.png" style=""></a></div>';
		if(ques.displayQuestionNo == "true" || ques.displayQuestionNo == "" || typeof(ques.displayQuestionNo) == "undefined"){
			str += '<div class="divHeader">';
			str +='<b><span class="questionNumber">'+mockLabels.questionNo+'</span>. '+eval(iOAP.curQues+1)+'</b>';
			str +='</div>';
...
function changeGroup(id){
	if(mockVar.curQuesBean.keyboardType == "AudioFile" || mockVar.curQuesBean.keyboardType == "VideoFile"){
		if(recordingInProgress){
			...
			$('.popClose').html($(globalXmlvar).find('popClose').text());
...
function submitConfirmation(param){
	...
	if(param == 'submit'){
		...
		timeOutStr +="<div  onclick='submitMock()' type='button' class='btn btn-primary nextbtn'><span><span id='timeoutSummaryNextBtnDiv '><strong>Next</span><img id='forwordImage' src='{{ URL_ROOT }}/static/Uscholar/img/Forward-25.png' width='25' height='25' /></strong></span></div></div>";
		return timeOutStr;
	}else if(param == 'break'){
//		for(var j=0;j<mockVar.groups.length;j++)
...
function moveToFeedback(){
	...
	if(mockVar.isFeedBackRequired == "NO"){
		window.location.href = "close.html?"+mockVar.orgId +"@@"+mockVar.mockId+"@@"+mockVar.subscribedFor+"#"+((document.cookie.indexOf('isApp=true')>= 0)?"isApp":"");
	}else{
...
function fillMaxOptQuesCrossed(quesToBeConsidered,totalQuestions){
	...
	str += '<table><tr><td style="vertical-align:middle"><img src="{{ URL_ROOT }}/static/Uscholar/img/warning-icon.png" /></td>';
...
function timeOutSubmit(){
	...
	//$('#rightDivision').html('<div style="top:25%;position:relative"><img src="{{ URL_ROOT }}/static/Uscholar/img/NewCandidateImage.jpg"  /></div>');
	isfinalSubmit = true;
	if($('.canvasmenu').is(':visible')){
		$("#minwidth").removeClass("animatewidth");
		$(".hamburgerBar").removeClass("hambg");
...
function submitOnlineAssessment(){
	if(DataError.ERROR.indexOf('Assessment has been submitted successfully.')!=-1){
		...
		if((document.cookie.indexOf('isApp=true')>= 0) ? true : false){
			window.location.href = "close.html?"+mockVar.orgId +"@@"+mockVar.mockId+"@@"+mockVar.subscribedFor+"#isApp";
		}else
		window.close();
		...
		setTimeout(function(){
			if(url.indexOf("quiz.html") > 0 && !isfinalSubmit && !isPlayer && mockVar.storeCandResponse == 1 && mockVar.curQuesBean.keyboardType != 'AudioFile' && mockVar.curQuesBean.keyboardType != 'VideoFile' && mockVar.curQuesBean.keyboardType != 'FileUpload'){
				...
				var candMasterId = $.trim(params[1]).split("@@")[3];*/
								...
								explicitClose=true;
								// window.close();
								window.location.href ="error.html?E125"+((document.cookie.indexOf('isApp=true')>= 0)?"isApp":"");
							/*}*/
						},10000);
					}	else if(allowInterruptions=='NO'){
						updateInterruptions(parseInt(noOfInterruptions+""),"Locked");
						toErrorPage=true;
						window.location.href ="error.html?E116#"+errorPageContent+"@@"+mockVar.lockedSubmit+"@@"+((document.cookie.indexOf('isApp=true')>= 0)?"isApp":" ");
					}
				}
			} else {
				isPlayer = false;
...
function makQuesAudVidDiv(quesText, oldFileCounter, quesId) {
	...
	if (quesText.indexOf("tkc{{ URL_ROOT }}/static/Uscholar/img/") != -1) {
		...
		for (var k = 1; k < tempImageTag.length; k++) {
			...
			quesText = quesText.replace(temporaryFileName, tempImageName);
			if (temporaryFileExtension == "mp4") {
				quesText = quesText + "<div class='jwAudioVideo' id='jwVideo_" + quesId + "_" + oldFileCounter + "' title='tkc{{ URL_ROOT }}/static/Uscholar/img/" + tempImageName + "'></div>";
				oldFileCounter++;
			} else if (temporaryFileExtension == "mp3") {
				quesText = quesText + "<div class='jwAudioVideo' id='jwAudio_" + quesId + "_" + oldFileCounter + "' title='tkc{{ URL_ROOT }}/static/Uscholar/img/" + tempImageName + "'></div>";
				oldFileCounter++;
			}
		}
	}
	if (quesText.indexOf("<audio>") != -1) {
		...
		for (var i = 0; i < newAudioFile.length; i++) {
			while (quesText.indexOf(newAudioFile[i]) != -1) {
				quesText = quesText.replace(newAudioFile[i], "<div class='jwAudioVideo' id='jwAudio_" + quesId + "_" + oldFileCounter + "' title='tkc{{ URL_ROOT }}/static/Uscholar/img/" + newAudioFile[i].split("<audio>")[1] + "'></div>");
				oldFileCounter++;
			}
		}
	}
	if (quesText.indexOf("<video>") != -1) {
		...
		for (var i = 0; i < newVideoFile.length; i++) {
			while (quesText.indexOf(newVideoFile[i]) != -1) {
				quesText = quesText.replace(newVideoFile[i], "<div class='jwAudioVideo' id='jwVideo_" + quesId + "_" + oldFileCounter + "' title='tkc{{ URL_ROOT }}/static/Uscholar/img/" + newVideoFile[i].split("<video>")[1] + "'></div>");
				oldFileCounter++;
			}
		}
	}
		...
		if(newAudioFile!=null){
			for (var i = 0; i < newAudioFile.length; i++) {
				while (content.indexOf(newAudioFile[i]) != -1) {
					content = content.replace(newAudioFile[i], "<div class='UsefuljwAudioVideo' id='jwAudio"+counter.value()+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/" + newAudioFile[i].split("<audio>")[1] + "'></div>");
					counter.increment();
				}
			}
		}
		...
		if(newVideoFile!=null){
			for (var i = 0; i < newVideoFile.length; i++) {
				while (content.indexOf(newVideoFile[i]) != -1) {
					content = content.replace(newVideoFile[i], "<div class='UsefuljwAudioVideo' id='jwVideo"+counter.value()+"' title='tkc{{ URL_ROOT }}/static/Uscholar/img/" + newVideoFile[i].split("<video>")[1] + "'></div>");
					counter.increment();
				}
			}
		}
	...
	if(str1.indexOf("imageZoom")>-1){
		if(i=="add"){
			$(answers[0]).children('span').children('span').each(function(){
				images=images+jQuery(this).children('a').attr('href').split('tkc{{ URL_ROOT }}/static/Uscholar/img/')[1]+",";
				imageCount++;
			});
		}else{
